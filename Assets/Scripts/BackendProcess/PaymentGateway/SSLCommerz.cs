...
namespace SSLCommerz.Web.PaymentGateway
{
    ...
    public class SSLCommerzGatewayProcessor 
    {
        ...
        protected string Marchen_URL = "validator/api/merchantTransIDvalidationAPI.php";
        protected string getsessioniD;
        public Transaction newroot;
        ...
        public string InitiateTransaction(NameValueCollection PostData, bool GetGateWayList = false)
        {
            ...
            try
            {
                ...
                this.getsessioniD = resp.sessionkey;
                UnityEngine.Debug.Log(resp.sessionkey);
        ...
        public IEnumerator CheckValidated()
        {
            
            string validatedIdUrl = $"https://sandbox.sslcommerz.com/validator/api/merchantTransIDvalidationAPI.php?sessionkey={getsessioniD}&store_id={Store_ID}&store_passwd={Store_Pass}&format=json";
            using (UnityWebRequest request = UnityWebRequest.Get(validatedIdUrl))
            {
                request.method = "GET";
                ...
                {
                    string jsonText = request.downloadHandler.text.ToString();
                    UnityEngine.Debug.Log(jsonText);
                    newroot = new JavaScriptSerializer().Deserialize<Transaction>(jsonText);
                    UnityEngine.Debug.Log(newroot.status);
        ...
        public bool OrderValidate(string MerchantTrxID, string MerchantTrxAmount, string MerchantTrxCurrency, HttpRequest req)
        {
            ...
            if (hash_verified)
            {
                ...
                string EncodedValID = UnityWebRequest.EscapeURL(req.Form["val_id"]);
                string EncodedStoreID = UnityWebRequest.EscapeURL(this.Store_ID);
                string EncodedStorePassword = UnityWebRequest.EscapeURL(this.Store_Pass);
        ...
        public static string Post(string uri, NameValueCollection PostData)
        {
            ...
            {
                string[] value = PostData.GetValues(key);
                if (value != null)
                {
                    foreach (string value2 in value)
                    {
                        form.AddField(key, value2);
                    }
                }
            }
            ...
            UnityEngine.Debug.Log(response + "For ME");
            return System.Text.Encoding.UTF8.GetString(response);
        }
        public  IEnumerator SendForm( string uri ,NameValueCollection collection )
        {
            WWWForm form = new WWWForm();
            foreach(string key in collection.AllKeys)
            {
                string[] value = collection.GetValues(key);
                if(value != null)
                {
                    foreach(string value2 in value)
                    {
